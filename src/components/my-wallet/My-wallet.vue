<template>
  <div id="wallet">
    <transition name="childtranslate">
      <router-view class="wallet-childview"></router-view>
    </transition>
    <div class="header">
    ...
    <div class="wallet-content">
      ...
      <ul class="select-pay">
        <li v-for="item in list">
          {{item.text}} <input type="radio" name="pay" v-model="item.check">
        </li>
      </ul>
      <button @click="charge">充值</button>
    </div>
  </div>
</template>
...
.wallet-content p:nth-child(2){
  margin-top: .3rem;
  color: #828384;
  font-size: .16rem;
}
.wallet-content p:nth-child(3){
  margin-top: .15rem;
  font-size: .5rem;
}
.select-pay{
